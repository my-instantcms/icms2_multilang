<?php
	...
	$hide_current = false; // скрыть текущий язык из списка

	$langs = cmsCore::getLanguages();
	if($hide_current) { unset($langs[array_search($user_lang, $langs)]); }
...
<div class="lang_switcher">
	...
	<?php if ($visible_items){?>
		<?php foreach($visible_items as $l){ ?>
			<?php $class = ($user_lang == $l) ? 'noswitch' : 'switch'; ?>
			<a class="<?php html($class); ?>" id="<?php html($l); ?>">
				<img src="<?php html($ico); ?><?php html($l); ?>.png" alt="<?php html($l); ?>"> 
				<?php html($l); ?>
			</a>
		<?php } ?>
	<?php } ?>
